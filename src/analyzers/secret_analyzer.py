...
SECRET_PATTERNS: Dict[str, List[Tuple[str, str, Severity]]] = {
    'aws': [
        ...
        (r'aws[_-]?secret[_-]?access[_-]?key\s*=\s*["\']?[A-Za-z0-9/+=]{40}["\']?', 'AWS Secret Access Key', Severity.HIGH),
    ],
    'github': [
    ...
    'openai': [
        ...
        (r'openai[_-]?api[_-]?key\s*=\s*["\']?\w+["\']?', 'OpenAI API Key', Severity.HIGH),
    ],
    'google': [
    ...
    'generic': [
        (r'api[_-]?key\s*=\s*["\']?[^"\'\s]{16,}["\']?', 'Generic API Key', Severity.MEDIUM),
        (r'api[_-]?secret\s*=\s*["\']?[^"\'\s]{16,}["\']?', 'API Secret', Severity.HIGH),
        (r'password\s*=\s*["\'][^"\']{8,}["\']', 'Hardcoded Password', Severity.HIGH),
        (r'secret[_-]?key\s*=\s*["\']?[^"\'\s]{16,}["\']?', 'Secret Key', Severity.HIGH),
        (r'token\s*=\s*["\']?[^"\'\s]{16,}["\']?', 'Token', Severity.MEDIUM),
        (r'private[_-]?key\s*=\s*["\']?[-]+BEGIN', 'Private Key', Severity.HIGH),
        (r'auth[_-]?token\s*=\s*["\']?[^"\'\s]{16,}["\']?', 'Auth Token', Severity.MEDIUM),
    ],
}
